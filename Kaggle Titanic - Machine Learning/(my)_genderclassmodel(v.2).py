...
def algorithm(train, test):
    ...
    ##decision table
    decision_t = np.zeros([2,n_classes,n_prices],float)
        ...
        for pprice in xrange(n_prices):
            for psex in xrange(2):
                element = train[(train[0::,sex] == psex) \
                    ...
                    & (train[0:,price].astype(np.float) // price_bin== pprice), 0]                
                decision_t[psex, pclass_i, pprice] = np.mean(element)
    ...
    for p in test:
        output.append(decision_t[p[sex-1].astype(np.int), p[pclass-1].astype(np.int), int(p[price-1] // price_bin)])
...
write_in_file('Data/(my)_genderclassmodel(v.2).csv', test, output)
